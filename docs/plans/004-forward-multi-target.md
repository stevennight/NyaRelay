...
当前 node 的有效配置流程是“HTTP 拉取失败后尝试本地缓存，WebSocket 断线后持续重连”。签名配置包含 30 天离线租约；租约到期后，node 会停止旧 relay runtime。较旧的 node 二进制在滚动升级期间继续收到 15 分钟兼容租约，直到它们声明支持 30 天租约。

这有利于可用性，但会带来控制面撤销延迟：controller 离线时不能立即撤销旧配置或旧 node。离线租约到期前，节点只能继续使用最后一份有效配置；controller 恢复后，node 重新连接并取得新签名配置。
